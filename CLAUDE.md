...
- [`docs/hangman-wheel-panda.md`](docs/hangman-wheel-panda.md) — Hangman Wheel Panda (Slice 18): a Wheel-of-Fortune-style hangman Games-tab game. **Server-authoritative throwaway play** (secret word lives in the session, never sent to the browser until the round ends; no attempts/reflection/feedback — like Roll the Dice). A 10-part panda is drawn one piece per wrong guess (face→body→hands→legs→eyes→nose→mouth; mouth = loss). **Spin-gated**: letters lock until you spin the **wheel**, and recognized slice phrases grant smart effects (extra guesses / reveal a letter / lose a guess via server `effect` action), any other custom text = 1 guess. **standard** wheels are super_admin-only + global; **custom** wheels are tutor-owned + tenant-scoped (`hangman_wheels`, `tutor_id` server-side, cross-tenant 404). Puzzles are organised into **categories** (20 themed + "Wowlo Mix") in `config/hangman-words.php`; `start` picks a **random category** (student doesn't choose) and shows its name as a hint. Puzzles may be **phrases** — only A–Z letters are guessed, spaces/punctuation are shown free. Web-Audio spin/win/lose sounds; emerald/amber accent. **Built.**
