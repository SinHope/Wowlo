    ...
    <div class="mx-auto max-w-5xl space-y-5">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-muted">{{ $messages->total() }} sent</p>
            </div>
            <a href="{{ route('tutor.messages.create') }}"
               ...
               class="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition-colors duration-200 hover:border-primary/40 hover:bg-primary/5 cursor-pointer">
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @if ($message->is_read)
                            <span class="rounded-full bg-success/10 px-2 py-0.5 text-xs font-semibold text-success">Read</span>
                        @else
                            <span class="rounded-full bg-amber/10 px-2 py-0.5 text-xs font-semibold text-amber-600">Unread</span>
                        @endif
                    </div>
        ...
        @empty
            <div class="rounded-2xl border border-gray-100 bg-white py-16 text-center shadow-sm">
                ...
                <p class="mt-1 text-sm text-muted">Compose your first message to a student.</p>
            </div>
        @endforelse
