<x-app-layout>
    <x-slot name="header">{{ $attempt->student?->name ?? 'Student' }} — Multiplication</x-slot>
    ...
    <div class="mx-auto max-w-2xl space-y-5">
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 text-center shadow-sm">
            <a href="{{ route('tutor.games.multiplication.index') }}" class="float-left text-sm font-semibold text-teal-700 hover:underline">&larr; All rounds</a>
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <h3 class="mb-4 text-base font-extrabold text-ink">Questions</h3>
                    ...
                    <li class="flex items-start gap-3 py-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-bold text-ink">
                                {{ $r['a'] }} &times; {{ $r['b'] }} =
                                <span class="{{ $r['is_correct'] ? 'text-success' : 'text-danger' }}">{{ $r['response'] !== null ? $r['response'] : '—' }}</span>
                            </p>
                            @unless ($r['is_correct'])
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('tutor.games.multiplication.feedback', $attempt) }}" class="mt-3">
                @csrf
                ...
                @enderror
                <button type="submit" class="mt-3 rounded-xl bg-teal-500 px-5 py-2.5 text-sm font-extrabold text-white transition-colors duration-200 hover:bg-teal-600 cursor-pointer">
