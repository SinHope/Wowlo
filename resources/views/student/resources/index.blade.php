    ...
    <div class="mx-auto max-w-4xl space-y-5">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-muted">
                    @if ($type === 'mcq')
                        Fill in an OAS your tutor sent, or make your own to record answers to a paper.
                    @else
                        Write answers your tutor sent, or make your own to record answers to a paper.
                    @endif
                </p>
            </div>
            <a href="{{ route('student.resources.create', $type) }}"
            ...
            <div class="flex items-center gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                ...
                <div class="flex shrink-0 flex-col items-end gap-2">
                    @if ($sheet->isMarked())
                        ...
                        <a href="{{ route('student.resources.show', $sheet) }}" class="text-xs font-semibold text-primary hover:underline cursor-pointer">View results &rarr;</a>
                    @elseif ($sheet->isSubmitted())
                        ...
                        <a href="{{ route('student.resources.show', $sheet) }}" class="text-xs font-semibold text-primary hover:underline cursor-pointer">View &rarr;</a>
                    @else
                        <span class="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-muted">To do</span>
                        <a href="{{ route('student.resources.show', $sheet) }}" class="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-white hover:bg-primary-dark cursor-pointer">Fill in</a>
                    @endif
                </div>
            </div>
        @empty
            <div class="rounded-2xl border border-gray-100 bg-white py-16 text-center shadow-sm">
                ...
                <p class="mt-1 text-sm text-muted">Your tutor hasn't sent any, and you haven't made one yet.</p>
            </div>
        @endforelse
