<x-app-layout>
    ...
    @if ($attempt->hasReflection())
        ...
        <div class="mx-auto max-w-2xl space-y-5">
            ...
            <a href="{{ route('student.games.spelling.progress') }}" class="inline-block text-sm font-semibold text-primary hover:underline">&larr; My Progress</a>

            @include('student.games.spelling.partials.results', ['attempt' => $attempt])
    ...
    @else
        {{-- ===================== GATE MODE (reflection still required) =====================
             ...
             rest of the app is blocked — but it SHOWS the marks + per-word review first, then
             the reflection box at the bottom. The student must write something to continue;
             only then does the normal page (with tutor feedback) unlock. --}}
        <div x-data="{ text: @js(old('reflection', '')) }"
            ...
            <div class="mx-auto max-w-2xl space-y-5 p-4 sm:p-6">
                ...
                <div class="rounded-2xl border-2 border-accent/40 bg-accent/5 p-6 shadow-sm">
                    <h3 class="text-base font-extrabold text-ink">My Reflection / Learning Points</h3>
                    <p class="mt-1 text-sm text-gray-600">
                        Now that you've seen your results — what did you find tricky? What will you remember next time?
                        <span class="font-semibold text-accent-dark">You can't go anywhere else until you write something (even a few words is fine).</span>
                    </p>
