...
/**
 * Spelling Meow — tutor side (see docs/spelling-game.md).
 *
...
class SpellingController extends Controller
{
    ...
    {
        $attempts = SpellingAttempt::where('tutor_id', auth()->id())
            ->with('student')
            ->latest('id')
            ->paginate(20);

        return view('tutor.games.spelling.index', compact('attempts'));
    }
    ...
    public function show(SpellingAttempt $attempt): View
    {
        ...
        return view('tutor.games.spelling.show', compact('attempt'));
    }

    /** Save the tutor's feedback and notify the student. */
    public function feedback(Request $request, SpellingAttempt $attempt): RedirectResponse
    {
        $this->ensureOwned($attempt);
        ...
        return redirect()->route('tutor.games.spelling.show', $attempt)
    ...
    private function notifyStudent(SpellingAttempt $attempt): void
    {
        $message = Message::create([
            'sender_id'   => auth()->id(),
            'receiver_id' => $attempt->student_id,
            'subject'     => "Spelling feedback: {$attempt->level}",
            'body'        => auth()->user()->name . " left feedback on your {$attempt->level} spelling round.",
        ]);
